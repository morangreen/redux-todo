...
window.add = function () {
  ...
  _store2.default.dispatch({ type: _actionTypes.ADD, data: { item: newItem } });
};

window.remove = function (index) {
  _store2.default.dispatch({ type: _actionTypes.REMOVE, data: { index: index } });
};

_store2.default.subscribe(function () {
  // const items = store.getState().todo.items;
  // const html = document.getElementById('tmpl-todo-list').innerHTML;
  // const tmpl = doT.template(html);
  var result = (0, _todoList2.default)({ items: items });

  document.getElementById('todo-items').innerHTML = result;
});
...
var combinedReducers = (0, _redux.combineReducers)({ todo: _todoReducer2.default });

exports.default = (0, _redux.createStore)(combinedReducers);
...
/**/) {
var out=' ';var arr1=it.items;if(arr1){var value,index=-1,l1=arr1.length-1;while(index<l1){value=arr1[index+=1];out+=' <li> <span>'+(value)+'</span> <a onclick="remove('+(index)+')"> X </a> </li> ';} } return out;
}
